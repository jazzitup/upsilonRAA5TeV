    ...
    ) 
{
  ...
  RooRealVar x1s("x1s","sigma fraction 1s 2nd CB", 0.5, 0., 2.05);
  ...
  //integrated, y<2.4
  RooRealVar alpha1s("alpha1s","tail shift", 1.6026, 1.1321, 4.647);
  RooRealVar n1s("n1s","power order", 2.9649, 2.282, 3.6771);
  ...
  //RooRealVar n1s("n1s","power order", 2.1 , 1.1, 5.9);
  ...
  RooRealVar *sl1 = new RooRealVar("sl1","sl1",0.1,-2.,3.);
  RooRealVar *cnst1 = new RooRealVar("cnst1","cnst1",0.1,-3,3.0);
  ...
  ws->pdf("model")->plotOn(myPlot2,Components(RooArgSet(*cb1s_1)),LineColor(kRed),LineWidth(2.7));
  ws->pdf("model")->plotOn(myPlot2,Components(RooArgSet(*cb1s_2)),LineColor(kGreen+1),LineWidth(2.7));
...
*/
  ...
  ws->pdf("model")->paramOn(legFrame,Layout(0.13,0.36,0.77),Parameters(paramList));
  ...
  if(collId == kPPMCUps1S) CMS_lumi(pad1, 200 ,33); 
  else if(collId == kAAMCUps1S) CMS_lumi(pad1, 201 ,33); 
  pad1->Update();
  ...
  outh->Write();
  c1->SaveAs(Form("fitresults_upsilon_DoubleCB_%s.pdf",kineLabel.Data()));
  c1->Write();
  ws->Write();
  outf->Close();
} 
