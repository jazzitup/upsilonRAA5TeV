...
int draw_sigparam_pt(TString szAA = "PP", int states =1)
{
  ...
  for (int ib =0; ib < nBin; ib ++ ) {
    //// read files
    if (szAA == "PP" ) { fileIn[ib]= new TFile(Form("./NomPlot_MC/fitresults_upsilon_DoubleCB_%s_MC_Ups1S_pt%.1f-%.1f_y0.0-2.4_muPt4.0.root",szAA.Data(),binArr[ib],binArr[ib+1])); }
    else if (szAA == "AA" ) { fileIn[ib]= new TFile(Form("./NomPlot_MC/fitresults_upsilon_DoubleCB_%s_MC_Ups1S_pt%.1f-%.1f_y0.0-2.4_muPt4.0_centrality0-200_dphiEp_0.00PI_100.00PI.root",szAA.Data(),binArr[ib],binArr[ib+1])); }
...
//    ws[ib]= (RooWorkspace*)fileIn[ib]->Get("workspace");
    if (szAA == "PP" ) { ws[ib]= (RooWorkspace*)fileIn[ib]->Get(Form("workspace_%s_MC_Ups1S_pt%.1f-%.1f_y0.0-2.4_muPt4.0",szAA.Data(),binArr[ib],binArr[ib+1])); }
    else if (szAA == "AA" ) { ws[ib]= (RooWorkspace*)fileIn[ib]->Get(Form("workspace_%s_MC_Ups1S_pt%.1f-%.1f_y0.0-2.4_muPt4.0_centrality0-200_dphiEp_0.00PI_100.00PI",szAA.Data(),binArr[ib],binArr[ib+1])); }
    //ws[ib]->Print();
    //// get parameters
    //cout << ws[ib]->var("alpha1s")->getVal() << endl;
    alpha1s[ib]=ws[ib]->var("alpha1s")->getVal();
    alpha1sErr[ib]=ws[ib]->var("alpha1s")->getError();
    n1s[ib]=ws[ib]->var("n1s")->getVal();
    n1sErr[ib]=ws[ib]->var("n1s")->getError();
