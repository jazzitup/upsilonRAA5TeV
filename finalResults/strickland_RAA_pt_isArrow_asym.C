...
void strickland_RAA_pt_isArrow_asym(bool isArrow=true)
{
  setTDRStyle();
...
//  leg_strick->Draw("same");
  ...
  CMS_lumi_raaCent( c1, iPeriod, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("Strickland_RAA_vs_pt_isArrow%d_asym.pdf",(int)isArrow));
  c1->SaveAs(Form("Strickland_RAA_vs_pt_isArrow%d_asym.png",(int)isArrow));
