...
void strickland_RAA_cent_newglobal_all_asym(bool isArrow =true)
{
  setTDRStyle();
...
//  double relsys = 0.1;
  ...
  for (int is=0; is<nState; is++){
    fInSys_Hi[is] = new TFile(Form("../Systematic/mergedSys_ups%ds_asymHi.root",is+1),"READ");
    fInSys_Lo[is] = new TFile(Form("../Systematic/mergedSys_ups%ds_asymLo.root",is+1),"READ");
...
//  CMS_lumi( c1, iPeriod, iPos );
  ...
  globtex->DrawLatex(0.5*(1-0.032*600/xlonger), sz_init-sz_step*2-sz_allign, "0-100%");
	...
	c1->Update();
  c1->SaveAs(Form("Strickland_RAA_vs_cent_isArrow%d_all_newglobal_asym.png",(int)isArrow));

  c1->SaveAs(Form("Strickland_RAA_vs_cent_isArrow%d_all_newglobal_asym.pdf",(int)isArrow));
/*
	///////////////////////////////////////////////////////////////////
	//// save as a root file
