...
void draw_RAA_pt_isArrow_asym(bool isArrow=true)
{
  setTDRStyle();
...
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi_raaCent( c1, iPeriod, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("RAA_vs_pt_isArrow%d_asym.pdf",(int)isArrow));
  c1->SaveAs(Form("RAA_vs_pt_isArrow%d_asym.png",(int)isArrow));
