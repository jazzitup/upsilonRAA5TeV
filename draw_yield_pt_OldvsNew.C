...
int draw_yield_pt_OldvsNew(int states =1, TString ColId = "PP")
{
  ...
  h2_nSig1s->GetYaxis()->SetRangeUser(1,100000);
  ...
  c_nSig1s->SaveAs(Form("yield/pt_nSig1s_%s_%s_%dS.pdf",szAA.Data(),ColId.Data(),states));
  ...
  latex->SetTextColor(kBlue+2);
  latex->DrawLatex(0.55,0.73,Form("(Old) #Upsilon(%dS) yields",states));
  c_nSig2s->SaveAs(Form("yield/pt_nSig2s_%s_%s_%dS.pdf",szAA.Data(),ColId.Data(),states));
  ...
  c_nSig3s->SaveAs(Form("yield/pt_nSig3s_%s_%s_%dS.pdf",szAA.Data(),ColId.Data(),states));
