...
int draw_bkgparam_cent(TString szAA = "AA", int states =1)
{
  /////////////////////////////////////////////////////////
  //// set style
  /////////////////////////////////////////////////////////
  ...
  double tmpArr1s[10] = {0, 10, 20, 40, 60, 80, 100, 120, 140, 200};
  double tmpArr2s[10] = {0, 10, 20, 40, 60, 80, 100, 120, 140, 200};
  double tmpArr3s[3] = {0, 60, 200};

  int tmpBin;
  if ( states ==1) {
    cout << " ***** 1S *****" << endl; tmpBin = 9;
  }else if (states ==2){
    cout << " ***** 2S *****" << endl; tmpBin = 9;
  }else if (states ==3){
  ...
  for (int ib =0; ib < nBin; ib ++ ) {
    ...
    cout << ib << "th lambda = " << lambda[ib] << ", #pm" << lambdaErr[ib] <<endl;
    cout << ib << "th mu = " << mu[ib] << ", #pm" << muErr[ib] <<endl;
    cout << ib << "th sigma = " << sigma[ib] << ", #pm" << sigmaErr[ib] <<endl;
  }
 
  //// histogram
  TH1D* h1_lambda = new TH1D("h1_lambda","h1_lambda;Centrality x 2 (%);#lambda",nBin,binArr); 
  TH1D* h1_mu = new TH1D("h1_mu","h1_mu;Centrality x 2 (%);#mu",nBin,binArr); 
  TH1D* h1_sigma = new TH1D("h1_sigma","h1_sigma;Centrality x 2 (%);#sigma",nBin,binArr); 
  ...
  c_lambda->SaveAs(Form("bkgparam/cent_lambda_%s_%dS.pdf",szAA.Data(),states));
  ...
  h1_mu->Draw("pe");
  latex->DrawLatex(0.45,0.83,Form("%s, binning for #Upsilon(%dS)",szAA.Data(),states));
  c_mu->SaveAs(Form("bkgparam/cent_mu_%s_%dS.pdf",szAA.Data(),states));
  ...
  h1_sigma->Draw("pe");
  latex->DrawLatex(0.45,0.83,Form("%s, binning for #Upsilon(%dS)",szAA.Data(),states));
  c_sigma->SaveAs(Form("bkgparam/cent_sigma_%s_%dS.pdf",szAA.Data(),states));
