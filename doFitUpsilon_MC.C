		     ...
		     ) 
{
  ...
  int   nMassBin  = (massHigh-massLow)*50;
  ...
  if ( collId == kPPMCUps1S) {  
    f0 = new TFile("/home/jazzitup/analysis/upsilonRAA/skimmedFiles/yskimPP_MC_Ups1S_Trig-L1DoubleMu0_OpSign_EP-OppositeHF_20168121653_ce8f82aaf8e612dcd1c7c161216161d988fbf9a6.root");
  }
  else if ( collId == kAAMCUps1S) {  
    f0 = new TFile("/home/jazzitup/analysis/upsilonRAA/skimmedFiles/yskimAA_MC_Ups1S_Trig-L1DoubleMu0_OpSign_EP-OppositeHF_2016812170_ce8f82aaf8e612dcd1c7c161216161d988fbf9a6.root");
  }
  /*
 ...
 ws->var("mass")->Print();
  ...
  RooRealVar sigma1s_1("sigma1s_1","width/sigma of the signal gaussian mass PDF",0.1, 0.01, 0.3);
  ...
  RooRealVar alpha1s("alpha1s","tail shift", 5. , 1.0, 9.8);
  RooRealVar alpha2s("alpha2s","tail shift", 5. , 1.15, 9.2);  
  RooRealVar alpha3s("alpha3s","tail shift", 5. , 1.11, 9.8);
  
  RooRealVar n1s("n1s","power order", 5. , 1.4, 9.8);
  ...
  RooRealVar *nSig1s= new RooRealVar("nSig1s"," 1S signals",10000,0,1000000);
  ...
  if ( (collId==kPPMCUps1S) || (collId==kAAMCUps1S) ) {
    model = new RooAddPdf("model","1S",RooArgList(*cb1s),RooArgList(*nSig1s));
  }
...
//  RooFitResult* fitRes2 = ws->pdf("model")->fitTo(*reducedDS,Save(), Hesse(kTRUE),Range(massLow, massHigh),Minos(0), SumW2Error(kTRUE));
  ...
  ws->pdf("model")->plotOn(myPlot2,Components(RooArgSet(*cb1s_2)),LineColor(kBlue),LineStyle(kDashed));
