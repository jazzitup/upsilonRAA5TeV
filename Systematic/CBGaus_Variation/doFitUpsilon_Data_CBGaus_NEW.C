       ...
       bool fixParameters=0)  // DON'T FIX PARAMETERs  
{
  ...
  RooRealVar    sigma1s_1("sigma1s_1","width/sigma of the signal gaussian mass PDF", 0.05, 0.01, 0.8);
  //RooRealVar    sigma1s_1("sigma1s_1","width/sigma of the signal gaussian mass PDF", initPset.sigma1s_1*0.9, 0.02, 0.3);
  ...
  RooRealVar *x1s = new RooRealVar("x1s","sigma ratio ", 0.6, 0.1, 3.2);
  //RooRealVar *x1s = new RooRealVar("x1s","sigma ratio ", initPset.x1s*0.9, initPset.x1s*0.80, initPset.x1s*1.2);
  ...
  RooRealVar err_mu("#mu","err_mu",init_mu,  0, 50);
  RooRealVar err_sigma("#sigma","err_sigma", init_sigma, 0,50);
  RooRealVar m_lambda("#lambda","m_lambda",  init_lambda, 0,50);

  ///cms/home/goni/work/Analysis/Upsilon_Raa_8_0_24/src/usercode/upsilonRAA5TeV/fitResults/NominalFits
...
*/  //RooRealVar m_lambda("#lambda","m_lambda",  init_lambda, 0,25);
  ...
  */
  ...
  cout<<"check2"<<endl;
  if(ptLow < 6){
  err_mu.setVal(getparam.mu);
  err_mu.setConstant();   
  err_sigma.setVal(getparam.sigma);
  err_sigma.setConstant();
  }
  cout<<"check3"<<endl;
  m_lambda.setVal(getparam.lambda);
...
*/
  ...
  TFile* outf = new TFile(Form("new_171207/PAS_fitresults_upsilon_DoubleCB_%s.root",kineLabel.Data()),"recreate");
  outh->Write();
  c1->SaveAs(Form("new_171207/PAS_fitresults_upsilon_DoubleCB_%s.pdf",kineLabel.Data()));
  c1->Write();
  ws->Write();
//  outf->Close();
...
valErr getParam(int collId, float ptLow, float ptHigh, float yLow, float yHigh, int muPtCut, int cLow, int cHigh, float dphiEp2Low,  float dphiEp2High) {
  ...
  TFile* inf = new TFile(Form("/cms/ldap_home/goni/work/Analysis/Upsilon_Raa_8_0_24/src/usercode/upsilonRAA5TeV/fitResults/NominalFits/PAS_fitresults_upsilon_%sCB_%s.root",SignalCB.Data(),kineLabel.Data()));
  RooWorkspace* ws = (RooWorkspace*)inf->Get("workspace");
  valErr ret;
  if(ptLow<6){
  ret.mu = ws->var("#mu")->getVal();
