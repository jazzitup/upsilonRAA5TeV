...
void computeSysUncrt_bkgPDF(int state=2)
{ 
  ...
  for ( int icent = 0 ; icent<= nCentBins ; icent++ ) 
  {
    ...
    if(icent == 0) aa = getRMS("fitresults_upsilon_DoubleCB_AA_DATA_pt0.0-30.0_y0.0-2.4_muPt4.0_centrality0-200_dphiEp_0.00PI_100.00PI",state);
    else 
    {
...
//  TH1D *hUncPtPP = new TH1D("hUncPtAA",";Uncertainty;p_{T} (GeV/c
...
double getRMS(TString fileName , int state) 
{
  ...
  { 
    if(ivar ==0) fin = Form("../../plots/TESTPLOT_FORNOMINAL/%s.root",fileName.Data());
