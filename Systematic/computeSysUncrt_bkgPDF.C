...
{ 
  ...
  for ( int icent = 0 ; icent<= nCentBins ; icent++ ) 
  {
    ...
    if(icent == 0) aa = getRMS("fitresults_upsilon_DoubleCB_AA_DATA_pt0.0-30.0_y0.0-2.4_muPt4.0_centrality0-200_dphiEp_0.00PI_0.50PI");
    else 
    {
      if(centBin[icent] <= 60) aa = getRMS(Form("fitresults_upsilon_DoubleCB_AA_DATA_pt0.0-30.0_y0.0-2.4_muPt4.0_centrality%d-%d_dphiEp_0.00PI_0.50PI", centBin[icent-1], centBin[icent]),state );
      else if(centBin[icent] > 60) aa = getRMS(Form("fitresults_upsilon_DoubleCB_AA_DATA_PeriL1_pt0.0-30.0_y0.0-2.4_muPt4.0_centrality%d-%d_dphiEp_0.00PI_0.50PI", centBin[icent-1], centBin[icent]),state );
    }
  ...
  for ( int ipt = 1 ; ipt<= nPtBins ; ipt++ ) {
    ...
    aa = getRMS(Form("fitresults_upsilon_DoubleCB_AA_DATA_pt%.1f-%.1f_y0.0-2.4_muPt4.0_centrality0-200_dphiEp_0.00PI_0.50PI", ptBin[ipt-1], ptBin[ipt] ),state ); // PbPb
    finalUnc = sqrt( pp*pp + aa*aa );
  ...
  for ( int iy = 1 ; iy<= nYBins ; iy++ ) {
    ...
    aa = getRMS(Form("fitresults_upsilon_DoubleCB_AA_DATA_pt0.0-30.0_y%.1f-%.1f_muPt4.0_centrality0-200_dphiEp_0.00PI_0.50PI", yBin[iy-1], yBin[iy]),state ); // PbPb
    finalUnc = sqrt( pp*pp + aa*aa );
    cout << "Unc. of double ratio  " << 1.000000* double(100*finalUnc) << "%%" << endl;
  }
...
double getUncVar(TString fileName , int state) 
{
  double yield[nVar+1];
  ...
  { 
    ...
    else fin = Form("BkgVariation/Sys_BkgVar_%s.root", fileName.Data());
