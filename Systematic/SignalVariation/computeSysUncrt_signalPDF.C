...
double getMaxDev(TString fileName = "DR_SignalSys/", int state=1);
...
{ 
  ...
  for ( int ipt = 1 ; ipt<= nPtBins ; ipt++ ) 
  {
    ...
    pp = getMaxDev(Form("fitresults_upsilon_DoubleCB_PP_DATA_pt%.1f-%.1f_y0.0-2.4_muPt4.0", ptBin[ipt-1], ptBin[ipt]), state); // pp
    aa = getMaxDev(Form("fitresults_upsilon_DoubleCB_AA_DATA_pt%.1f-%.1f_y0.0-2.4_muPt4.0_centrality0-200_dphiEp_0.00PI_100.00PI", ptBin[ipt-1], ptBin[ipt] ),state ); // PbPb
    finalUnc = sqrt( pp*pp + aa*aa );
  ...
  for ( int iy = 1 ; iy<= nYBins ; iy++ ) 
  {
    ...
    pp = getMaxDev(Form("fitresults_upsilon_DoubleCB_PP_DATA_pt0.0-30.0_y%.1f-%.1f_muPt4.0", yBin[iy-1], yBin[iy]),state ); // pp
    aa = getMaxDev(Form("fitresults_upsilon_DoubleCB_AA_DATA_pt0.0-30.0_y%.1f-%.1f_muPt4.0_centrality0-200_dphiEp_0.00PI_100.00PI", yBin[iy-1], yBin[iy]),state ); // PbPb
    finalUnc = sqrt( pp*pp + aa*aa );
    cout << "Unc. of double ratio  " << 1.000000* double(100*finalUnc) << "%%" << endl;
  }
...
double getMaxDev(TString fileName , int state) 
{
  double yield[nVar+1];
  double relVar[nVar+1];   // relative variance
  double MaxDev;
