...
void doFitUpsilon_Data_signalSys(
       int collId = kAADATA,  
       float ptLow=0, float ptHigh=30, 
       float yLow=0, float yHigh=2.4,
       int cLow=0, int cHigh=200,
       float muPtCut=4.0,
       bool paramToRelease = kN_ )
{
  float dphiEp2Low = 0 ;
  ...
  // Fix? 
  if ( paramToRelease != kN_ )    
    n1s_1.setConstant();
  if ( paramToRelease != kAlapha_ )
    alpha1s_1.setConstant();
  if ( paramToRelease != kSigma_ )
    sigma1s_1.setConstant();
  if ( paramToRelease != kF_ )
    f1s->setConstant();
  if ( paramToRelease != kX_ )
    x1s->setConstant();
