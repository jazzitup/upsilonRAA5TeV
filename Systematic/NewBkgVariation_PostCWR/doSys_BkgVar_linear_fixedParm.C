...
void doSys_BkgVar_linear_fixedParm(
       int collId = kAADATA,  
       ...
       bool fixParameters=1  )
{
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", 2. , 1.00, 10.84);
  ...
  RooRealVar n1s_1("n1s_1","power order", 2.1 , 1.00, 10);
  ...
  RooRealVar *fbkg = new RooRealVar("fbkg","1S CB fraction", 0.5, 0, 1);
  
  RooGenericPdf *bkg_nom;
  ...
  if  (ptLow >= 6)        bkg_nom = bkgHighPt ;
  else bkg_nom = bkgLowPt;


  /*RooRealVar sl("sl","sl",3.63,-31,31);
  RooRealVar cnst("cnst","cnst",210,0,580);
  */

  // AA y 1S bin, 50-60% 70-100%, integrated (+pp), pt 0-2, 2-4, 4-6  
  RooRealVar sl("sl","sl",0.63,-45,45); 
  RooRealVar cnst("cnst","cnst",210,0,730);
  
  RooGenericPdf *bkg_linear = new RooGenericPdf("bkglinear","bkglinear","@0*@1+@2",RooArgList( *(ws->var("mass")),sl,cnst) );

  RooAddPdf *bkg = new RooAddPdf("bkg","background",RooArgList(*bkg_nom,*bkg_linear),RooArgList(*fbkg) );
...
*/
  ...
  TFile* outf = new TFile(Form("Sys2_fitresults_upsilon_DoubleCB_%s.root",kineLabel.Data()),"recreate");
  outh->Write();
  c1->SaveAs(Form("Sys2_fitresults_upsilon_DoubleCB_%s.pdf",kineLabel.Data()));
  c1->Write();
  ws->Write();
