...
const int nVar = 5;
double *getPar(TString fileName = "filename");
void doSys_BkgVar_4th_fixedParm(
       int collId = kAADATA,  
       ...
       bool fixParameters=1  )
{
  ...
  RooRealVar    sigma1s_1("sigma1s_1","width/sigma of the signal gaussian mass PDF",0.02, 0.01, 0.5);
  ...
  RooRealVar *x1s = new RooRealVar("x1s","sigma ratio ", 0.5, 0, 5);
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", 2.2 , 1.00, 10.64); // AA pT 0-4 , 0-5%
  //RooRealVar alpha1s_1("alpha1s_1","tail shift", 2. , 1.15, 3.84);
  ...
  RooRealVar n1s_1("n1s_1","power order", 3.12 , 1.00, 10.56);
  ...
  RooRealVar *f1s = new RooRealVar("f1s","1S CB fraction", 0.5, 0., 1); // AA pT 0-4, 0-5%
  ...
  filename = Form("fitresults_upsilon_DoubleCB_%s",kineLabel.Data());
  double* SigPar;
  SigPar = getPar(filename);
  ...
  if ( initPset.n1s_1 == -1 )
    ...
    n1s_1.setVal(SigPar[0]);  
    alpha1s_1.setVal(SigPar[1]);
    sigma1s_1.setVal(SigPar[2]);
    f1s->setVal(SigPar[3]); 
    x1s->setVal(SigPar[4]);
  } 
  // Fix? 
  ...
  // for almost every bin
  /*
  RooRealVar ch4_k1("ch4_k1","ch4_k1",-0.1,-1.1,1.) ;
  RooRealVar ch4_k2("ch4_k2","ch4_k2",0.1,-1.,1.) ;
  RooRealVar ch4_k3("ch4_k3","ch4_k3",0.1,-1.1,1.2) ;
  RooRealVar ch4_k4("ch4_k4","ch4_k4",0.1,-1.,1.) ;
*/

  // PP y 1.6-2.0, AA 50-60%
 /* RooRealVar ch4_k1("ch4_k1","ch4_k1",-0.1,-1.1,1.) ;
  RooRealVar ch4_k2("ch4_k2","ch4_k2",0.1,-1.,1.) ;
  RooRealVar ch4_k3("ch4_k3","ch4_k3",-0.1,-1.1,1.1) ;
  RooRealVar ch4_k4("ch4_k4","ch4_k4",-0.1,-1.,1.) ;
*/
  // AA 0-5%, pT 0-4
  RooRealVar ch4_k1("ch4_k1","ch4_k1",0.1,-1.1,1.) ;
  RooRealVar ch4_k2("ch4_k2","ch4_k2",0.1,-1.1,1.) ;
  ...
  RooRealVar *nBkg = new RooRealVar("nBkg","fraction of component 1 in bkg",0,2000000);  
...
*/
  ...
  TFile* outf = new TFile(Form("Sys1_fitresults_upsilon_DoubleCB_%s.root",kineLabel.Data()),"recreate");
  outh->Write();
  c1->SaveAs(Form("Sys1_fitresults_upsilon_DoubleCB_%s.pdf",kineLabel.Data()));
  c1->Write();
  ws->Write();
...
double *getPar(TString fileName) 
{
  double* signalparam = new double[nVar];
  TString fin;
  fin = Form("/home/deathold/work/CMS/analysis/Upsilon_RAA/upsilonRAA5TeV/fitResults/Final_NomResult_170124/PAS_%s.root",fileName.Data());
  TFile* f1 = new TFile(fin.Data() );
  RooWorkspace* ws = (RooWorkspace*)f1 -> Get("workspace");
  signalparam[0] = ws->var("n1s_1")->getVal();
  signalparam[1] = ws->var("alpha1s_1")->getVal();
  signalparam[2] = ws->var("sigma1s_1")->getVal();
  signalparam[3] = ws->var("f1s")->getVal();
  signalparam[4] = ws->var("x1s")->getVal();
  return signalparam;
}
