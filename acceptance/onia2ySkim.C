...
void onia2ySkim( int nevt = -1,
    int fileID = kPPMCUps3S, // kAAMCUps1S
  ...
  // dmoon added weighted function
  TFile *fUp1Spp = new TFile("../WeightedFcN/ratioDataMC_PP_DATA_1sState.root","READ");
  TFile *fUp2Spp = new TFile("../WeightedFcN/ratioDataMC_PP_DATA_2sState.root","READ");
  TFile *fUp1Spb = new TFile("../WeightedFcN/ratioDataMC_AA_DATA_1sState.root","READ");
  TFile *fUp2Spb = new TFile("../WeightedFcN/ratioDataMC_AA_DATA_2sState.root","READ");
  ...
  if( (fileID == kPPMCUps1S) ) {
    wFunc[1] = (TF1*)fUp1Spp->Get("dataMcRatio_iy1");
    wFunc[2] = (TF1*)fUp1Spp->Get("dataMcRatio_iy2");
  }else if( (fileID == kPPMCUps2S) || (fileID == kPPMCUps3S)) {
    wFunc[1] = (TF1*)fUp2Spp->Get("dataMcRatio_iy1");
    wFunc[2] = (TF1*)fUp2Spp->Get("dataMcRatio_iy2");
  }
  ...
  if (fileID == kPPDATA) {
    ...
    fname = "../../MCFiles/OniaTree_Ups1SMM_5p02TeV_TuneCUETP8M1_nofilter_pp502Fall15-MCRUN2_71_V1-v1_GENONLY.root";
    ...
    fname = "../../MCFiles/OniaTree_Ups2SMM_5p02TeV_TuneCUETP8M1_nofilter_pp502Fall15-MCRUN2_71_V1-v1_GENONLY.root";
    ...
    fname = "../../MCFiles/OniaTree_Ups3SMM_5p02TeV_TuneCUETP8M1_nofilter_pp502Fall15-MCRUN2_71_V1-v1_GENONLY.root";
  ...
  */  
  ...
  for(int iev=0; iev<nevt ; ++iev)
  {
    ...
    for (Int_t irqq=0; irqq<Reco_QQ_size; ++irqq) 
    {
      ...
      if(dm.mass>=7.5 && dm.mass<=14)
      {
        DIMUPASS_all++;

        if( (fileID == kAADATA) || (fileID == kAADATAPeri) || (fileID == kAADATACentL3) || (fileID == kAAMC) || (fileID == kAAMCUps1S) || (fileID == kAAMCUps2S) || (fileID==kAAMCUps3S) )
        {
          if(dm.cBin>=0 && dm.cBin <10)
            DIMUPASS_CENT1++;
          else if(dm.cBin>=10 && dm.cBin<20)
            DIMUPASS_CENT2++;
          else if(dm.cBin>=20 && dm.cBin<40)
            DIMUPASS_CENT3++;
          else if(dm.cBin>=40 && dm.cBin<60)
            DIMUPASS_CENT4++;
          else if(dm.cBin>=60 && dm.cBin<80)
            DIMUPASS_CENT5++;
          else if(dm.cBin>=80 && dm.cBin<100)
            DIMUPASS_CENT6++;
          else if(dm.cBin>=100 && dm.cBin<120)
            DIMUPASS_CENT7++;
          else if(dm.cBin>=120 && dm.cBin<140)
            DIMUPASS_CENT8++;
          else if(dm.cBin>=140 && dm.cBin<160)
            DIMUPASS_CENT9++;
        }
      }
