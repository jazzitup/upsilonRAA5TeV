...
void getAcceptance(int bin = 1, int doDraw = 1) { // doDraw == 1 : draw, == 0 : no draw
  ...
  const int nPtBins2 = 5;   double ptBin2[nPtBins2+1] = {0,2.5,5.0,8.0,12.0,30};  const int nYBins2 = 1;   double yBin2[nYBins2+1] = {0,2.4};
  ...
  const int nPtBins4 = 3;   double ptBin4[nPtBins4+1] = {0,5.0,15.0,30};  const int nYBins4 = 1;   double yBin4[nYBins4+1] = {0,2.4};
  const int nPtBins5 = 1;   double ptBin5[nPtBins5+1] = {0,30};  const int nYBins5 = 2;   double yBin5[nYBins5+1] = {0,1.2,2.4};
...
float getAcceptanceSingleBin(int state, int icoll, TString kineCut, int varianceID) {
  ...
  //// all GEN
  mmGen->Draw(Form("y*weight>>%s",hGen->GetName()), Form("(%s)*(%s)", kineCut.Data(), ptWeight.Data()));
  //// GEN with sglMuAccCut 
  mmGen->Draw(Form("y*weight>>%s",hGenAcc->GetName()), Form("((%s)&&(%s))*(%s)", kineCut.Data(), accCut.Data(), ptWeight.Data()));
