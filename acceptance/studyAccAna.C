...
void studyAccAna(int Cat_ = 0) { // Cat_ == 0 (1S),  Cat_ == 1 (2S), Cat_ == 2 (3S)
  ...
  const int nPtBins4 = 3;   double ptBin4[nPtBins4+1] = {0,4,9,30};         const int nYBins4 = 1;   double yBin4[nYBins4+1] = {0,2.4}; // 2S
  ...
  if(Cat_ == 0) in = new TFile("skimedForAcc_MC_Ups1S_20170606.root");
  ...
  if(Cat_ == 0) out = new TFile("acceptance_wgt_1S_20170111.root","RECREATE");
  ...
  for(int i = 0; i < nEntries; i++){
    ...
    if(fabs(y) < 2.4&&pt<30.0){
      ...
      hIntAccNoW1->Fill(fabs(y));
      hptAccNoW1->Fill(pt);
      hrapAccNoW1->Fill(fabs(y));
      ...
      if(pt1>4.0 && pt2>4.0 && fabs(eta1) < 2.4 && fabs(eta2) < 2.4){
        hIntAccNoW2->Fill(fabs(y));
        hptAccNoW2->Fill(pt);
        hrapAccNoW2->Fill(fabs(y));
  ...
  TF1 *fline = new TF1("fline","1",0.0,30);
