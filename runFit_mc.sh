...
outputDir='fitResults/mcFit_MuPt'${muPtCut}'_2016_08_30'

#outputDir='Test/fixParam'${fixParam}'MuPt'${muPtCut}'_fullRap_mar01_bkgtest'${fbkg_ch}''
mkdir -p $outputDir
cp onia2ySkim.C doFitUpsilon_MC.C PsetCollection.h cutsAndBin.h $outputDir
...
for i in ${kPPMCUps1S} ${kAAMCUps1S}  # 0=kPPDATA, 2=kAADATA 6=kAADATAPeri 7=kAADATACentL3 8=kPPMCUps1S 9=kPPMCUps2S
do
	...
	do
	    ...
	    ./condor_root.sh $outputDir 'doFitUpsilon_MC.C('$i','$pt','$rap',0,200,'$muPtCut')'
	done
    done
done
